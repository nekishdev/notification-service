...
class NotificationSourceChoices(models.TextChoices):
    MAIL = "mail", _("mail")
    SMS = "sms", _("sms")
...
class TemplatesBody(TimeStampedMixin, UUIDMixin):
    ...
    content_type = models.CharField(
        _("content_type"),
        max_length=255,
        choices=BodyTemplatesContentTypeChoices.choices,
    )
    description = models.TextField(_("description"), blank=True, null=True)
    conversion_options = models.TextField(
        _("conversion_options"), blank=True, null=True
    )  # for PDF
...
class TemplatesNotify(TimeStampedMixin, UUIDMixin):
    ...
    body = models.ForeignKey(
        "TemplatesBody", on_delete=models.CASCADE, db_index=False, blank=True, null=True
    )
    ...
    send_from = models.ForeignKey(
        "Senders", on_delete=models.CASCADE, db_index=False, blank=True, null=True
    )
...
class Recipients(TimeStampedMixin, UUIDMixin):
    ...
    is_read = models.CharField(
        _("is_read"), max_length=255, choices=YesNoChoices.choices
    )
...
class Notifications(TimeStampedMixin, UUIDMixin):
    notification_templates = models.ForeignKey(
        "TemplatesNotify", on_delete=models.CASCADE, db_index=False
    )
...
class Message(TimeStampedMixin, UUIDMixin):
    ...
    send_at = models.DateTimeField(
        "send at", blank=False, null=False, auto_now_add=True
    )
    status = models.CharField(
        _("status"),
        max_length=255,
        blank=False,
        null=False,
        choices=MessagesStatusChoices.choices,
        default=MessagesStatusChoices.WAIT,
    )
